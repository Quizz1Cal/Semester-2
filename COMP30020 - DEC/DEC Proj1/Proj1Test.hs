...
loop answer guess other guesses = do
    ...
    if validSelection guess && length answer == length guess then do
        ...
        if guesses > 2 then putStrLn $ "(From gamestate) :" ++ show other else return ()
        if length guess < 3 && guesses > 1 then putStrLn $ "(From gamestate) :" ++ show other else return ()
        if successful guess result then do
            ...
            -- putStrLn $ "Answered wrong, using " ++ show result ++ "\nwith gamestate" ++ show other ++ "\nfrom guess" ++ show guess
            let (guess',other') = nextGuess (guess,other) result
            -- putStrLn $ ">> Your new state is " ++ show other'
            -- putStrLn $ ">> New guess is " ++ show guess'
